...
prompt_and_setup_ssl() {
    ...
    if [[ "${PANEL_USE_DOMAIN}" == "true" ]]; then
        ...
        # IP-based panel
        echo -e "${green}Panel configured with IP: ${PANEL_HOST}${plain}"
        echo ""
        ...
        if [[ "${ssl_choice}" == "1" ]]; then
            ...
            if is_ipv6 "${server_ip}"; then
                ...
                setup_ipv6_only_certificate "${server_ip}"
            else
                ...
                if [[ -n "${detected_ipv6}" ]]; then
                    ...
                    if [[ "${add_ipv6}" == "y" || "${add_ipv6}" == "Y" ]]; then
                        setup_ip_certificate "${server_ip}" "${detected_ipv6}"
                    else
